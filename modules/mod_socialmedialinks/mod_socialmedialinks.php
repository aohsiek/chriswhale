...
// Set Wrapping Div
	echo '<div class="smile" style="'. $alignstyle .'"><ul> ';
...
// Output the Icon Links	
	 	 if(($vimg[$ic[$i]]) != '') {
			...
			echo '<li><a class="'.strtolower($title).'" style="margin:0 '.$margin.';" '. $nofollow .' href="'. $vimg[$ic[$i]]. '" target="'. $target .'"><img src="'. $mod .'icons/'. $theme .'/'. $size .'/'. $vurl[$url[$i]] .' " alt="'. $title .'" '; if($title == 'Feed') { echo 'title="'. $rsstext .'" /><span>' . $rsstext .'</span></a></li>';}else{ echo 'title="'. $text .' '. $title .'" /><span>' . $title .'</span></a></li>';}
		 }
	 } 
		?>
	</ul>
