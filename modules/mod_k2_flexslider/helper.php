				...
				if ($format != 'feed')
				{
					...
					if ($params->get('JPlugins', 1))
					{
						...
						if (K2_JVERSION == '16')
						{
							...
							$dispatcher->trigger('onContentPrepare', array('mod_k2_flexslider', &$item, &$params, $limitstart));

							$results = $dispatcher->trigger('onContentAfterTitle', array('mod_k2_flexslider', &$item, &$params, $limitstart));
							$item->event->AfterDisplayTitle = trim(implode("\n", $results));

							$results = $dispatcher->trigger('onContentBeforeDisplay', array('mod_k2_flexslider', &$item, &$params, $limitstart));
							$item->event->BeforeDisplayContent = trim(implode("\n", $results));

							$results = $dispatcher->trigger('onContentAfterDisplay', array('mod_k2_flexslider', &$item, &$params, $limitstart));
					...
					foreach ($item->extra_fields as $key => $extraField)
					{
						if ($extraField->type == 'textarea' || $extraField->type == 'textfield')
						{
							...
							if ($params->get('JPlugins', 1))
							{
								if (K2_JVERSION == '16')
								{
									$dispatcher->trigger('onContentPrepare', array('mod_k2_flexslider', &$tmp, &$params, $limitstart));
								}
								else
								{
