...
class plgSystemRokCandy extends JPlugin {
	...
	function _shouldProcess() {
	    ...
	    if (JRequest::getCmd('layout') == 'edit' && $params->get('editenabled',0) == 0 && !JRequest::getCmd('option') == "com_user")
            return true;
	...
	function _replaceCode(&$body) {
	    ...
	    foreach ($this->_library as $key => $val) {
			...
			// remove the title 
			$find_title = "/(?:<title>)(.*?)(?:<\/title>)/";
			preg_match_all($find_title, $working_body, $title_tag_matches);
			foreach($title_tag_matches as $titletagbody) {
				if(!empty($titletagbody)){ 
					$working_body = str_replace($titletagbody,'',$working_body);
				}	
			}
