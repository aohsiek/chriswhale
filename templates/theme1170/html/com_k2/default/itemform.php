...
$templateDir = JPATH_THEMES.DS.JFactory::getApplication()->getTemplate().DS;
	...
	$detect = new Mobile_Detect();
		...
		$detectAndroid = $detect->isAndroidOS();
		...
		function homeButton(){
			echo '<a class="itemFormHome" href="index.php">' . JText::_("K2_HOME") .'</a>'; 
		}
?>

<style type="text/css">
	 @media only screen and (max-width: 959px) {
		td#toolbar-cancel{display:none;}
	 }
</style>
		...
		<div id="k2Frontend">
			<table class="k2FrontendToolbar" cellpadding="2" cellspacing="4">
				<tr>
					...
					<?php 
						if(($detectTablet == false) && ($detectMobile == false) && ($detectAndroid == false) && ($detectIphone == false)){
							cancelButton();
						} 	
					?>
				</tr>
			...
			<?php if(($detectTablet == true) || ($detectMobile == true) || ($detectAndroid == true) || ($detectIphone == true)){			
				homeButton();
			}
			?>
